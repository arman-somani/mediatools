...
async function downloadAndMergeViaPollingAPI(
  ...
  // Poll for up to 25 seconds (5 attempts * 5s) to avoid long waits
  let isReady = false;
  for (let i = 0; i < 5; i++) {
    try {
  ...
  if (!isReady) throw new Error('Polling API timed out waiting for file after 25 seconds');
        ...
        try {
          ...
          if (!videoDownloaded) {
            try {
              ...
              console.error('Alternative RapidAPI video failed:', e.message);
            }
          }
